...
$mkbot->cmd('*', 'Maaf perintah salah atau tidak tersedia');
...
$mkbot->cmd('/start|/Start', function () {
   ...
   if (has($idtelegram) == false) {
      ...
      $text    = 'Selamat datang ' . $Name_router;
      $options = [
         'parse_mode' => 'html'
      ];
...
// HAPUS VOUCHER
$mkbot->regex('/^\/hapus/', function ($matches) {
   $info         = bot::message();
   $msgid        = $info['message_id'];
   $nametelegram = $info['from']['username'];
   $idtelegram   = $info['from']['id'];
   ...
   if ($idtelegram == $id_own) {
      if ($isipesan == '/hapus') {
         ...
         $id  = str_replace('/hapus', '', $isipesan);
         $ids = str_replace('@' . $usernamebot, '', $id);
         $API = new routeros_api();

         if ($API->connect($mikrotik_ip, $mikrotik_username, $mikrotik_password, $mikrotik_port)) {
            // $arrayUser = $API->comm('/tool/user-manager/user/print', ['?username' => $ids]);
            // $ARRAY  = $API->comm("/ip/hotspot/user/print", ["?.id" => $ids,]);
            // $data1  = $ARRAY[0]['.id'];
            // $data2  = $arrayUser[0]['username'];
            // $data3  = $arrayUser[0]['password'];
            // $data5  = $arrayUser[0]['actual-profile'];
            // sleep(2);
            $ARRAY2 = $API->comm('/tool/user-manager/user/remove', ['numbers' => $ids]);
...
// MENU PAKET VOUCHER
$mkbot->cmd('!Menu|/Menu|/menu|menu', function () {
   $info         = bot::message();
   $ids          = $info['chat']['id'];
   $msgid        = $info['message_id'];
   $nametelegram = $info['from']['username'];
   ...
   if (has($idtelegram)) {
      ...
      if (!empty($data)) {
         $text .= "👇 Silahkan Pilih Paket dibawah ini\n\n";
      ...
      if (strpos($command, 'Vcr') !== false) {
         ...
         if (preg_match('/' . $command . '/i', $cekid)) {
               ...
               if ($command == 'Vcr' . $getid2) {
                  if (sisasaldo($id, $princevoc) == true) {
                     ...
                     if ($type == 'up') {
                        ...
                        $usernamereal = $prefix . make_string($length, $typechar);
                     ...
                     switch ($limituptime) {
                        ...
                        default:
                           ...
                           $echoexperid .= "<code>  AKTIF   :</code> <code>{$uptime}</code>\n";
                           break;
                     }
                     ...
                     if ($API->connect($mikrotik_ip, $mikrotik_username, $mikrotik_password, $mikrotik_port)) {
                        ...
                        // userman add voucher
                        $add_user_api = $API->comm('/tool/user-manager/user/add', array(
                           'customer'      => 'admin',
                        ...
                        if ($type == 'up') {
                           ...
                           // $caption .= $echoexperid;
                           $caption .= "<code>=========================</code>\n";
                           $caption .= "<code> TGL " . date('d/m/Y, H:i:s') . "</code>\n";
                           $caption .= "<code>=========================</code>\n";
                        } else {
                           $caption = "";
                           $caption .= "<code>=========================</code>\n";
                           ...
                           $caption .= "<code>=========================</code>\n";
                           $caption .= "<code>  MASA AKTIF  :</code> <code>$profile</code>\n";
                           // $caption .= $echoexperid;
                           $caption .= "<code>=========================</code>\n";
                           $caption .= "<code>  " . date('d/m/Y H:i:s') . "</code>\n";
                           $caption .= "<code>=========================</code>\n";
                        }
            ...
            if (!empty($ganguan)) {
               ...
               if ($API->connect($mikrotik_ip, $mikrotik_username, $mikrotik_password, $mikrotik_port)) {
                  $ARRAY2 = $API->comm("/tool/user-manager/user/remove", ["numbers" => $add_user_api,]);
               }
         ...
         $text = "";
         $text .= " Selamat datang bos   <i>$nama </i> \n\n";
         $text .= "<i>Silahkan Pilih voucher dibawah ini</i>\n\n";
         foreach ($data as $hargas) {
            $textlist = $hargas['Text_List'];
